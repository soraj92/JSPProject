...
<script type="text/javascript">
	...
	function openLogin(i) {
		var win = i;
		window.name = "parent";
		if (win == 'page/login.jsp' || win == '../page/login.jsp')
...
<div class="pos-f-t">
	...
	<nav class="navbar navbar-dark fixed-top"
		...
		<div class="collapse" id="navbarToggleExternalContent">
			...
			%>
			<font size="5em" color="white"><%=session.getAttribute("isLogin")%>님
				환영합니다.</font>>
			<form action="myPage.com" method="post">
				<input type="submit" class="btn btn-primary" value="내정보" />
			</form>
			<%
			...
			<a role="button" aria-disabled="true"><button
					...
					onclick="window.open('../page/logout.jsp')">로그아웃</button></a>
			<%
				} else {
			%>
			<a role="button" aria-disabled="true"><button
					class="btn btn-secondary" onclick="window.open('page/logout.jsp')">로그아웃</button></a>
			<%
				}
			%>
			<%
			...
			%>
			<button class="btn btn-primary" onclick="openLogin('page/login.jsp')">로그인</button>
			<a role="button" aria-disabled="true"><button
					class="btn btn-secondary"
					onclick="window.open('page/register.jsp')">회원가입</button></a>
			<%
